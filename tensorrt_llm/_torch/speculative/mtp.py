...
class B10Decoder:
    ...
    @staticmethod
    def get_custom_sampling_params(scheduled_requests: ScheduledRequests,
        ...
        for i, request in enumerate(itertools.chain(scheduled_requests.context_requests,
            ...
            if temperature is not None and not math.isclose(temperature, B10Decoder.DEFAULT_TEMPERATURE):
    ...
    @staticmethod
    def batch_decode(model_outputs,
                          *,
                          request_idx,
                          logits_idx,
                          temperature,
        ...
        lens = model_outputs['new_tokens_lens'] if 'new_tokens_lens' in model_outputs else (
            torch.ones_like(model_outputs['new_tokens'], dtype=torch.int32)
        )
...
class MTPDecoder(TorchDecoder):
    ...
    def decode_async(self, scheduled_requests: ScheduledRequests,
        ...
        inputs = B10Decoder.get_custom_sampling_params(scheduled_requests, model_outputs['logits'].device)
        request_idx, sampled_token_offset, sampled_tokens = B10Decoder.batch_decode(model_outputs, **inputs)
